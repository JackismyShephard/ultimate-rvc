...
def render(
    ...
    ----------
    total_config : TotalConfig
        The total configuration object containing the settings for the
        Ultimate RVC model.
    ...
    with gr.Tab("Multi-step generation"):
        for input_track in tab_config.input_tracks:
    ...
    with gr.Accordion("Step 0: song retrieval", open=True):
        ...
        retrieve_song_btn.click(
            ...
            partial(
                update_dropdowns,
                get_named_song_dirs,
                len(tab_config.song_dirs) + 2,
                value_indices=range(len(tab_config.song_dirs)),
            ),
            inputs=current_song_dir,
            outputs=(
                [
                    *tab_config.song_dirs,
                    tab_config.cached_song.instance,
            ...
            outputs=total_config.management.audio.intermediate_audio.instance,
            show_progress="hidden",
        )

        song_transfer_btn.click(
            partial(update_audio, len(tab_config.input_tracks)),
            inputs=[song_transfer, song_output],
            outputs=[c.instance for c in tab_config.input_tracks],
            show_progress="hidden",
        )


def _render_step_1(tab_config: MultiStepGenerationConfig) -> None:
    with gr.Accordion("Step 1: vocal separation", open=False):
        gr.Markdown("")
        gr.Markdown("**Inputs**")
        tab_config.audio_track_input.instance.render()
        tab_config.separate_audio_dir.instance.render()
        gr.Markdown("**Settings**")
        with gr.Row():
        ...
        separate_vocals_btn.click(
            ...
            inputs=[
                tab_config.audio_track_input.instance,
                tab_config.separate_audio_dir.instance,
                tab_config.separation_model.instance,
                tab_config.segment_size.instance,
            ],
...
def _render_step_2(tab_config: MultiStepGenerationConfig) -> None:
    with gr.Accordion("Step 2: vocal conversion", open=False):
        gr.Markdown("")
        gr.Markdown("**Inputs**")
        tab_config.vocals_track_input.instance.render()
        with gr.Row():
            tab_config.convert_vocals_dir.instance.render()
        ...
        convert_vocals_btn.click(
            ...
            inputs=[
                tab_config.vocals_track_input.instance,
                tab_config.convert_vocals_dir.instance,
        ...
        converted_vocals_transfer_btn.click(
            partial(update_audio, len(tab_config.input_tracks)),
            inputs=[converted_vocals_transfer, converted_vocals_track_output],
            outputs=[c.instance for c in tab_config.input_tracks],
            show_progress="hidden",
        )


def _render_step_3(tab_config: MultiStepGenerationConfig) -> None:
    with gr.Accordion("Step 3: vocal post-processing", open=False):
        gr.Markdown("")
        gr.Markdown("**Inputs**")
        tab_config.converted_vocals_track_input.instance.render()
        tab_config.postprocess_vocals_dir.instance.render()
        gr.Markdown("**Settings**")
        with gr.Row():
        ...
        postprocess_vocals_btn.click(
            ...
            inputs=[
                tab_config.converted_vocals_track_input.instance,
                tab_config.postprocess_vocals_dir.instance,
                tab_config.room_size.instance,
                tab_config.wet_level.instance,
                tab_config.dry_level.instance,
            ],
        ...
        effected_vocals_transfer_btn.click(
            partial(update_audio, len(tab_config.input_tracks)),
            inputs=[effected_vocals_transfer, effected_vocals_track_output],
            outputs=[c.instance for c in tab_config.input_tracks],
            show_progress="hidden",
        )


def _render_step_4(tab_config: MultiStepGenerationConfig) -> None:
    with gr.Accordion("Step 4: pitch shift of background audio", open=False):
        gr.Markdown("")
        gr.Markdown("**Inputs**")
        with gr.Row():
            tab_config.instrumentals_track_input.instance.render()
            backup_vocals_track_input = gr.Audio(label="Backup vocals", type="filepath")
        tab_config.pitch_shift_background_dir.instance.render()
        gr.Markdown("**Settings**")
        with gr.Row():
        ...
        pitch_shift_instrumentals_btn.click(
            ...
            inputs=[
                tab_config.instrumentals_track_input.instance,
                tab_config.pitch_shift_background_dir.instance,
                tab_config.n_semitones_instrumentals.instance,
            ],
...
def _render_step_5(
    total_config: TotalConfig,
    tab_config: MultiStepGenerationConfig,
) -> None:
    with gr.Accordion("Step 5: song mixing", open=False):
        gr.Markdown("")
        gr.Markdown("**Inputs**")
        with gr.Row():
            tab_config.main_vocals_track_input.instance.render()
            tab_config.instrumentals_track_input.instance.render()
            tab_config.backup_vocals_track_input.instance.render()
        tab_config.mix_dir.instance.render()
        gr.Markdown("**Settings**")
        with gr.Row():
        ...
        with gr.Row():
            tab_config.output_name.instantiate(
                ...
                inputs=[
                    tab_config.main_vocals_track_input.instance,
                    tab_config.mix_dir.instance,
                ],
            )
        ...
        mix_btn.click(
            partial(
                ...
                [
                    tab_config.main_vocals_track_input.instance,
                    tab_config.instrumentals_track_input.instance,
                    tab_config.backup_vocals_track_input.instance,
                ],
                [
                    tab_config.main_gain.instance,
                    tab_config.inst_gain.instance,
            ...
            outputs=total_config.management.audio.output_audio.instance,
            show_progress="hidden",
        )
        song_cover_transfer_btn.click(
            partial(update_audio, len(tab_config.input_tracks)),
