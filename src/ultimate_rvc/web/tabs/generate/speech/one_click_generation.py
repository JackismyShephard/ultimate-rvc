...
from ultimate_rvc.web.typing_extra import (
    ConcurrencyId,
    OneClickSpeechGenerationConfig,
    SpeechSourceType,
    TotalConfig,
)
...
def render(total_config: TotalConfig) -> None:
    ...
    total_config : TotalConfig
        The configuration object containing all the settings for the
        application.
    ...
    with gr.Tab("One-click generation"):
        ...
        _render_edge_tts_options(tab_config)
        _render_speech_conversion_options(tab_config)
...
def _render_edge_tts_options(tab_config: OneClickSpeechGenerationConfig) -> None:
...
def _render_speech_conversion_options(
    tab_config: OneClickSpeechGenerationConfig,
) -> None:
    with gr.Accordion("Speech conversion options", open=False):
        ...
        tab_config.autotune_voice.instance.change(
            partial(
                toggle_visibility,
                targets={True},
                default=tab_config.autotune_voice.instance.value,
            ),
            inputs=tab_config.autotune_voice.instance,
            outputs=tab_config.autotune_voice.instance,
            show_progress="hidden",
        )
        tab_config.clean_voice.instance.change(
            partial(
                toggle_visibility,
                targets={True},
                default=tab_config.clean_voice.instance.value,
            ),
            inputs=tab_config.clean_voice.instance,
            outputs=tab_config.clean_voice.instance,
            show_progress="hidden",
        )
